...
export class XcoreService implements OnApplicationBootstrap, OnModuleDestroy {
  readonly bridge = createXcoreBridge({
    ...
    // STILL AUTHENTICATES, against the accounts lent under `di.accounts`. It
...
The seven routes go on as a middleware - they are the library's handlers, and the guard must not run in front of them: `/sso/start` is what a signed-out browser is sent to.
