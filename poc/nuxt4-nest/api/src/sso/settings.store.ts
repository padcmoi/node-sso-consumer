...
export class SettingsStore {
   ...
   */
  async all() {
    const rows = await this.db.select<SettingRow>("SELECT `key`, `type`, `value` FROM app_settings");
    ...
    for (const row of rows) {
      ...
      if (value === UNREADABLE) this.logger.warn(`${row.key} is not a readable ${row.type} and was ignored`);
